...
class ApiClient(object):
    ...
    def _parse(self, text):
        # print('Response:\n%s' % text)
        result = json.loads(text.decode(), object_hook=_toDict)
        if result.status=='ok':
            return result.data
...
def get_eth_ticker():
    ...
    ret = requests.get(SCHEME + "://" + API_HOST + '/market/kline', params=params)
    return ret.json()['tick']
...
def get_eth_depth():
    ...
    ret = requests.get(SCHEME + "://" + API_HOST + '/market/depth', params=params)
    return ret.json()['tick']
